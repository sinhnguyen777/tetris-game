...
class TetrisUI:
    def __init__(self, game):
        ...
        self.hold_title = self.title_font.render("Hold", True, (255, 255, 255))
        self.hold_rect = py.Rect(10, 55, 170, 170)
        self.score_rect = py.Rect(10, 240, 170, 60)
        self.lines_clear_surf = self.title_font.render(
            "Lines Clear", True, (255, 255, 255)
        )
        self.lines_clear_rect = py.Rect(520, 55, 170, 60)
        self.next_title = self.title_font.render("Next", True, (255, 255, 255))
        self.next_rect = py.Rect(520, 165, 170, 485)

        py.display.set_caption("Tetris")
        self.screen = py.display.set_mode((700, 700))

        self.playfield_countdown_surf = py.Surface((300, 600)).convert_alpha()
        self.playfield_countdown_surf.fill(py.Color(0, 0, 0, 100))
        self.playfield_background_surf = py.Surface((300, 600))
        self.playfield_background_surf.fill((26, 31, 40))
        self.game_surf = py.Surface((700, 700))
        self.game_surf.fill(py.Color("#C77DFF"))
        self.playfield_surf = py.Surface((300, 600), py.SRCALPHA)
        self.playfield_surf.fill(py.Color("#505050"))
        self.game = game
    ...
    def draw_countdown_screen(self, num):
        self.screen.blit(self.playfield_countdown_surf, (200, 50))
    ...
    def draw_screen(self):
        ...
        self.screen.blit(self.playfield_background_surf, (200, 50))
        self.screen.blit(self.playfield_surf, (200, 50))

        # Hold section
        self.screen.blit(
            self.hold_title,
            self.hold_title.get_rect(
                centerx=self.hold_rect.centerx,
                centery=self.hold_rect.centery
                - self.hold_rect.size[1] / 2
                - self.hold_title.get_height() / 2,
            ),
        )
        py.draw.rect(self.game_surf, "#aa77d1", self.hold_rect, 0, 10)
        self.game.draw_hold_tetromino(self.game_surf)
        ...
        py.draw.rect(self.game_surf, "#aa77d1", self.lines_clear_rect, 0, 10)
        ...
        self.screen.blit(
            self.next_title,
            self.next_title.get_rect(
                centerx=self.next_rect.centerx,
                centery=self.next_rect.centery
                - self.next_rect.size[1] / 2
                - self.next_title.get_height() / 2,
            ),
        )
        py.draw.rect(self.game_surf, "#aa77d1", self.next_rect, 0, 10)
        self.game.draw_current_queue(self.game_surf)

    def draw_game_elements(self):
        self.game.draw(self.playfield_surf)
    ...
    def draw_start_screen(self):
        ...
        game_over_surf = self.large_font.render("Tetris", True, (255, 0, 0))
        self.screen.blit(
            game_over_surf,
    ...
    def draw_settings_screen(
        ...
        values = [das_value, arr_value, soft_drop_speed, level]
        ...
        for i, option in enumerate(options):
            ...
            self.screen.blit(option_text, (100, 100 + i * 50))
